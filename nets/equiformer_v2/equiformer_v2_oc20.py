...
class EquiformerV2_OC20(BaseModel):
    ...
    def forward(self, data):
        ...
        # idea1
        edge_distance_raw = edge_distance
        density_scale_nodes = self.edge_degree_embedding.compute_density_scale(
            edge_distance_raw,
            edge_index,
            num_atoms,
        )
        edge_density_scale = density_scale_nodes.index_select(0, edge_index[1]).unsqueeze(-1)
        ...
        edge_distance = self.distance_expansion(edge_distance_raw)
        # idea1

        edge_distance = edge_distance * edge_density_scale
        if self.share_atom_edge_embedding and self.use_atom_edge_embedding:
