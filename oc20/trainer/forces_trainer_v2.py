...
class ForcesTrainerV2(BaseTrainerV2):
    ...
    def __init__(
        ...
        # idea2
        self.consistency_coefficient = self.config["optim"].get("consistency_coefficient", 0.0)
        self._latest_consistency_loss = None
        # idea2
    ...
    def train(self, disable_eval_tqdm=False):
        ...
        for epoch_int in range(
            ...
            for i in range(skip_steps, len(self.train_loader)):
                ...
                if self._latest_consistency_loss is not None:
                    log_dict["consistency_loss"] = self._latest_consistency_loss.item()
                if (
    ...
    def _forward(self, batch_list):
        # forward pass.
        if self.consistency_coefficient > 0 and self.model.training:
    ...
    def _compute_loss(self, out, batch_list):
        ...
        consistency_mult = self.consistency_coefficient
        if (
            consistency_mult > 0
            and self.model.training
            and (self.config["model_attributes"].get("regress_forces", True)
                 or self.config['model_attributes'].get('use_auxiliary_task', False))
        ):
            positions = [batch.pos for batch in batch_list]
            grad_outputs = torch.ones_like(out["energy"], device=out["energy"].device)
            ...
            energy_forces = []
            for grad, batch in zip(energy_grads, batch_list):
                if grad is None:
                    grad = torch.zeros_like(batch.pos)
                energy_forces.append(-grad)
            energy_forces = torch.cat(energy_forces, dim=0)

            if self.normalizer.get("normalize_labels", False):
                energy_std = self.normalizers["target"].std.to(energy_forces.device)
                energy_forces = energy_forces * energy_std.view(1, 1)
                predicted_forces = self.normalizers["grad_target"].denorm(out["forces"])
            else:
                predicted_forces = out["forces"]
            ...
            if self.config["task"].get("train_on_free_atoms", False):
                ...
                mask = fixed == 0
                consistency_vector = predicted_forces[mask] + energy_forces[mask]
            else:
                consistency_vector = predicted_forces + energy_forces

            consistency_loss = consistency_mult * torch.mean(consistency_vector.pow(2))
            loss.append(consistency_loss)
            self._latest_consistency_loss = consistency_loss.detach()
